...
def hash_token(raw_token: str) -> str:
	"""Hash a raw single-use token using SHA-256. Single-use secrets must
	never be stored in plaintext (NFR-SEC.3)."""
...
def verify_token(raw_token: str, hashed_token: str) -> bool:
	"""Verify a raw token against a stored SHA-256 hash using constant-time
	comparison (NFR-SEC.3)."""
	if not raw_token or not hashed_token:
		return False
	return secrets.compare_digest(hash_token(raw_token), hashed_token)
