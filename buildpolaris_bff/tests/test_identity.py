...
class TestModule1Identity(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		_bootstrap()  # idempotent: roles, custom fields, versioning

	def setUp(self):
		frappe.set_user("Administrator")

	# ---------------- UC-01 ----------------
	def test_uc01_register_creates_isolated_disabled_admin(self):
		suffix = _u()
	...
	def test_uc01_activation_enables_admin(self):
		suffix = _u()
	...
	def test_uc02_invite_assigns_roles_and_isolation(self):
	...
	def test_uc07_last_admin_cannot_be_demoted(self):
		admin_email, _ = self._make_admin()
		frappe.set_user(admin_email)
		with self.assertRaises(Exception):
			svc.update_user_roles(admin_email, ["BuildPolaris Project Manager"])

	def test_uc05_non_admin_cannot_invite(self):
		admin_email, _ = self._make_admin()
		frappe.set_user(admin_email)
	...
	def test_uc03_session_context_resolves_persona(self):
		admin_email, company = self._make_admin()
		frappe.set_user(admin_email)
